...
def test_agent_workflow_lcs():
    input_queue = [
        ...
        "Instead, write a test helper function `is_subsequence(sub, s)` that checks whether `sub` can be formed by "
        "deleting zero or more characters from `s` WITHOUT changing the order of the remaining characters — the "
        "characters of `sub` do NOT need to be contiguous in `s`. Do NOT implement this as `sub in s` or any other "
        "substring/containment check — that tests something different and will incorrectly fail on valid interleaved "
        "subsequences (e.g. is_subsequence('abc', 'xaxbxcx') must return True, even though 'abc' does not appear "
        "contiguously in 'xaxbxcx'). A correct implementation walks through `s` once, advancing a pointer into `sub` "
        "each time the current characters match. "
