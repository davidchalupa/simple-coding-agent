...
def pytest_addoption(parser):
    parser.addoption(
        ...
        action="store_true",
        default=False,
        help="Run tests using the StarCoder2 model."
    )
    parser.addoption(
        "--model",
        action="store",
        default="qwen2.5",
        help="Specify model key from MODEL_REGISTRY (e.g. qwen2.5, starcoder2)."
    )


@pytest.fixture(autouse=True)
def setup_agent_model(request):
    if request.config.getoption("starcoder"):
        selected_model = "starcoder2"
    else:
